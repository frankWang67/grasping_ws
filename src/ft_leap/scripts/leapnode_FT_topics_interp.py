...
# Refactored: services removed; state now published via topics
...
class LeapNode(Node):
  def __init__(self):
    ...
    self.create_subscription(
      JointState, "cmd_leap_interp", self._receive_pose_interp, 10
    )
    ...
    # Creates services that can give information about the hand out        #        #        # You can put the correct port here or have the node auto-search for a hand at the first 3 ports.
    ...
    # Timer to publish current state at fixed rate
  ...
  def _publish_state(self):
    ...
    self.positions = positions = positions.tolist()
    self.velocities = velocities = velocities.tolist()
    msg = JointState()
    msg.header.stamp = self.get_clock().now().to_msg()
    msg.name = [f"rh_joint_{m}" for m in self.motors] if hasattr(self, "motors") else []
    msg.position = (
      positions.tolist() if hasattr(positions, "tolist") else list(positions)
    )
    if velocities is not None:
      msg.velocity = (
        velocities.tolist() if hasattr(velocities, "tolist") else list(velocities)
      )
    ...
    # Start interpolation towards target position if enabled
    ...
    if self.interp_enable and elapsed_time <= self.interp_T:
      ...
      progress = 10 * t**3 - 15 * t**4 + 6 * t**5  # Smoothstep interpolation
