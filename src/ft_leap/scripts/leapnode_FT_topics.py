...
class LeapNode(Node):
    def __init__(self):
        ...
        for i in range(3):
            ...
            try:
                self.get_logger().info(
                    f"Attempting to connect to LEAP Hand on port {port}..."
                )
                ...
                self.get_logger().info(
                    f"Successfully connected to LEAP Hand on port {port}."
                )
                break
        ...
        self.leap_pos_vel_pub = self.create_publisher(JointState, "/irmhand/joint_states", 10)
